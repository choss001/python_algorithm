...
def solution(want, number, discount):
    ...
    while len(discount) > end:
        ...
        check_flag = check_all_true(check_true(dic_target, dic_solution, check_dic))
        if check_flag:
            result.append(start)
            start += 1
            end += 1
...
def sum_discount(discount):
    ...
    return dic_target
def check_true(dic_target, dic_solution, check_dic):
    for k,v in dic_target.items():
        if k in dic_solution:
            dic_solution[k] -= 1
            if dic_solution[k] <= 0:
                check_dic[k] = True
    return check_dic
